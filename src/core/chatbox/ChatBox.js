  ...
  return (
    <div className={styles.chatMain}>
      {value.map((value, index) => (
        <Box className={value.isClient ? styles.chatMain__right : styles.chatMain__left} sx={{borderRadius: value.isClient ? "10px 8px 0px 8px" : "8px 8px 8px 0px"}}>
          <img src={value.image} />
         <Box className={styles.chatMain__pad}>{value.message}</Box>
        </Box>
      ))}
      <input type='text' onChange={handleChange} value={input} placeholder='Type your message...' />
      <span>
        ...
        <img onClick={handleSend} style={{marginTop: "30px", marginLeft: "10px",  position: "absolute"}} src={send} alt="send"/>
